...
func ValidateResetToken(email, tokenString string) (*Claims, bool) {
	fmt.Println("Validating reset token:", tokenString)
	...
	fmt.Println("Claims expiration:", claims.ExpiresAt)
	...
	// Check if the email from claims matches the given email (only if email is provided)
	...
	fmt.Println("Token validation successful for email:", claims.Email)
	return claims, true
}
